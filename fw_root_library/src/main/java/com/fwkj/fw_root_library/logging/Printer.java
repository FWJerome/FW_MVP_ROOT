package com.fwkj.fw_root_library.logging;
...
class Printer {
    ...
    static void printJsonResponse(LoggingInterceptor.Builder builder, long chainMs, boolean isSuccessful,
        ...
        if (builder.getLevel() == Level.BASIC || builder.getLevel() == Level.BODY) {
            ...
            I.log(builder.getType(), tag, "Json" + replaceBlank(bodyString), builder.isLogHackEnable());
        }
        if (builder.getLogger() == null) {
            I.log(builder.getType(), tag, END_LINE, builder.isLogHackEnable());
        }
    ...
    static void printFileRequest(LoggingInterceptor.Builder builder, Request request) {
        ...
        if (builder.getLogger() == null)
            I.log(builder.getType(), tag, REQUEST_UP_LINE, builder.isLogHackEnable());
        logLines(builder.getType(), tag, new String[]{URL_TAG + request.url() + getBodyString(request)}, builder.getLogger(),
    ...
    private static String getBodyString(Request pRequest) {
        String message = "";
        RequestBody body = pRequest.newBuilder().build().body();
        if (body == null) {
            return "";
        }
        ...
        try {
            ...
            String s = requestbuffer.readString(charset);
            s = s.replaceAll("%(?![0-9a-fA-F]{2})", "%25");
            String decode = URLDecoder.decode(s, convertCharset(charset));
            try {
                decode = decode.trim();
